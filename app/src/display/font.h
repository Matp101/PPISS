/*
 * Standard ASCII 5x7 bitmap font — column-major format.
 * Derived from the classic Adafruit GFX glcdfont.c (BSD license).
 *
 * 256 characters × 5 bytes each = 1280 bytes.
 * Each byte is one column (left to right), bit 0 = top row.
 *
 * To render character 'c' at scale S:
 *   for col 0..4:
 *     byte = glcdfont[c * 5 + col]
 *     for row 0..6:
 *       pixel_on = (byte >> row) & 1
 */
...
static const uint8_t glcdfont[256 * 5] = {
